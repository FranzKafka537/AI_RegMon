...
# ---- Konfiguration ----
historic_file <- "regulatory_news_history_esg.csv"
...
thematic_focus <- "Regulatorische Entwicklungen und neue Vorgaben in den Bereichen ESG (Environmental, Social, Governance), insbesondere in Bezug auf ESG-Risikomanagement, Klimarisikomanagement, ESG-Compliance, nicht-finanzielle Berichterstattung und Internes Kontrollsystem (IKS). Der Fokus liegt auf der Relevanz für Finanzdienstleister (primär Banken) und der Bewertung der Auswirkungen auf Risikostrategie, Kontrollen und Offenlegungspflichten (z.B. TCFD, LkSG). Ziel ist es, Beratungsbedarfe und Handlungsempfehlungen für unsere Finanzkunden zu identifizieren."
...
empfaengerliste <- Sys.getenv("EMAIL_RECIPIENTS_ESG")
brevo_api_key <- Sys.getenv("BREVO_API_KEY")

if (sender_email == "" || empfaengerliste == "" || brevo_api_key == "") {
  warning("E-Mail-Zugangsdaten (EMAIL_ADDRESS, EMAIL_RECIPIENTS_ESG oder BREVO_API_KEY) fehlen. E-Mail-Versand wird übersprungen.")
...
<p style="font-size: 14px; line-height: 1.5; margin: 15px 0;">Fokus dieses KI-generierten Berichts: ESG-Risikomanagment für Finanzdienstleister.</p>
